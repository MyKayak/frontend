...
import TeamTile from "./team_tile"
import { TeamPreview } from "@/models/team"

import { loadMoreTeamsAction } from "@/app/actions"
...
export default function LoadMoreTeams({ initialOffset, query }: Props) {
  ...
  const loadMore = async () => {
    setLoading(true)
    try {
      const newTeams = await loadMoreTeamsAction(query, offset);
  ...
  if (!hasMore && teams.length === 0) return null
